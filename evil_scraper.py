...
class Other_Counties(object):
    ...
    def retrieve_ballot(self, browser, fpath = 'O:/Python Scripts/ev scraper/output/', fname = 'franklin.csv'):
        import random
        browser.get('https://ohiodems.org/vote')
        browser.find_element_by_id('pwbox-16665').send_keys('votefromhome')
        #I really hate these try/except blocks, but they fucking work.
        #There are three possible options. selenium throws an error if you ask it to select something that's not there.
        email_urls = ['@gmail.com', '@yahoo.com', '@hotmail.com', '@gmail.com', '@yahoo.com', '@hotmail.com','@gmail.com', '@yahoo.com', '@hotmail.com','@msn.com','@live.com','@zoom.net', '@scbglobal.net', '@aol.com']
        
        df = pd.read_csv(r'C:\Users\psbar\Documents\ev_scraper\input\sample_file.csv')
        browser.find_element_by_name('Submit').click()
        df = df[df['PARTY_AFFILIATION'] == 'R']
        first_names = df['FIRST_NAME']
        last_names = df['LAST_NAME']
        addresses = df['RESIDENTIAL_ADDRESS1']
        cities = df['RESIDENTIAL_CITY']
        zips = df['RESIDENTIAL_ZIP']
        ...
        browser.find_element_by_name('state_cd').send_keys('O')


        for i in range(len(first_names)):
            person = i
            email = last_name[person]+'_'+first_name[person]+str(random.randint(1,2005))+random.choice(email_urls)
            browser.find_element_by_name('email').send_keys(email.lower())
            browser.find_element_by_name('firstname').send_keys(first_name[person].title())
            browser.find_element_by_name('lastname').send_keys(last_name[person].title())
            browser.find_element_by_name('addr1').send_keys(address[person].title())
        
            browser.find_element_by_name('city').send_keys(city[person].title())
            browser.find_element_by_name('zip').send_keys(zip_code[person])
            ...
            browser.find_element_by_name('addr1').clear()
        
            browser.find_element_by_name('city').clear()
            browser.find_element_by_name('zip').clear()
...
def main(): 
    ...
    evil = Other_Counties(votefind.fpath)

    evil.retrieve_ballot(browser)
